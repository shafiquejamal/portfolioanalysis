...
object DatasetFixture extends PortfolioFixture {
  ...
  val investmentInputDataset: Dataset[ETFDataPlus] =
    Seq[ETFDataPlus](
  ...
  val expectedQuantities: Map[Int, Seq[FinalPortfolioQuantityToHave]] =
    Map(
      1 -> Seq(
  ...
  val expectedRebalancedPortfolio: Dataset[ETFDataPlus] = investmentInputDataset.map { eTFData =>
    if (eTFData.asOfDate isBefore startDatePlus3months)
      eTFData.copy(
        quantity = BigDecimal(expectedQuantities(1).find(_.eTFCode == eTFData.eTFCode).map(_.quantity).getOrElse(0)),
        cash = 9.0220
      )
    else if (eTFData.asOfDate isBefore startDatePlus6months)
      eTFData.copy(
        quantity = BigDecimal(expectedQuantities(2).find(_.eTFCode == eTFData.eTFCode).map(_.quantity).getOrElse(0)),
        cash = 1.5259217860353611027869343721905
      )
    else if (eTFData.asOfDate isBefore startDatePlus9months)
      eTFData.copy(
        quantity = BigDecimal(expectedQuantities(3).find(_.eTFCode == eTFData.eTFCode).map(_.quantity).getOrElse(0)),
        cash = 4.5557653581060833083608031165721
      )
    else
      eTFData.copy(
        quantity = BigDecimal(expectedQuantities(4).find(_.eTFCode == eTFData.eTFCode).map(_.quantity).getOrElse(0)),
